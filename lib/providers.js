    ...
    if (planLimit !== null && planLimit > 0) {
      ...
      if (remaining !== null) {
        ...
        entries.push({
          ...
          note: estimated ? "月度消费估算（套餐总额 − 周期消费）" : sub?.status ? `订阅 ${sub.status}` : "",
        });
        if (pct !== null && (tightest === null || pct > tightest)) tightest = pct;
      }
    }
...
function mergeOrgResults(usage, cost) {
  ...
  const headline = usage.headline && usage.state !== "err" && usage.state !== "off"
    ...
    : (cost.headline && cost.state !== "err" && cost.state !== "off"
        ...
        : { kind: "amt", amt: "—" });
  if (errors.length) {
    return { state: "err", error: { code: "partial", message: errors.join("；") }, entries, headline };
  }
...
export const OFFICIAL_HOSTS = {
  deepseek: DEEPSEEK.hosts,
  "moonshot-cn": ["api.moonshot.cn"],
  "moonshot-intl": ["api.moonshot.ai"],
  zai: ["api.z.ai"],
  "zai-cn": ["open.bigmodel.cn"],
  minimax: ["www.minimax.io"],
  "minimax-cn": ["www.minimaxi.com"],
  openrouter: OPENROUTER.hosts,
  "openrouter-account": OPENROUTER.hosts,
  "openai-org": ["api.openai.com"],
  "anthropic-org": ["api.anthropic.com"],
};
