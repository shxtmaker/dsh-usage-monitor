...
async function httpJson(url, { auth = "bearer", key = "", extraHeaders = {}, timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
  ...
  try {
    const response = await fetch(url, {
      signal: queryContext.getStore()
        ? AbortSignal.any([controller.signal, queryContext.getStore()]) : controller.signal,
      headers,
...
// ---------- 官方地址策略（唯一事实来源）----------
// 查询函数与自动探测校验必须引用同一份主机/基础路径策略：任何一方单独改动都会让
// 「探测可采纳的地址」与「查询可使用的地址」出现分歧。这里在注册表构造之前完成初始化，
// 避免依赖注册表求值顺序（历史上策略表放在文件末尾，靠 PROVIDERS 已求值才没出问题）。
...
/** 各官方供应商允许的主机与基础路径（无该键 = 私有兼容来源，不做官方地址约束）。 */
...
/**
 * 重置时刻的原始 epoch-毫秒（无时刻返回 null）。
 *
 * 与 formatReset 并存：formatReset 把「还有多久」四舍五入成整小时后塞进文案（最多 ±30 分钟误差），
 * 客户端要显示精确倒计时（如 2h13m / 1d19h）就必须拿到原始时刻自己算。
 * 只在供应商**确实给出**时刻时返回非空——绝不为了排版推算一个时刻出来。
 */
export function resetAtMs(resetsAt) {
  if (!resetsAt) return null;
  const target = resetsAt instanceof Date ? resetsAt : new Date(resetsAt);
  const ms = target.getTime();
  // 只接受 epoch-毫秒（> 1e9 = 2001-09 之后）。上游在「没有重置时刻」时会用
  // 0 / 1970 这类哨兵值占位（真实样本：Command Code windowLimits 缺失时 resetAt=0），
  // 那必须等价于「没给」——否则客户端会把 0 当成过期时刻，画出「即将重置」。
  return Number.isFinite(ms) && ms > 1e9 ? ms : null;
}
...
function makePctHeadline(tightest, entries) {
  if (tightest === null) {
    ...
    if (bal) return { kind: "amt", amt: bal.remain, reset: bal.reset, resetAt: bal.resetAt ?? null };
    const usage = entries.find((e) => e.kind === "usage" || e.kind === "cost");
    if (usage) return { kind: "amt", amt: usage.used, reset: usage.reset, resetAt: usage.resetAt ?? null };
    return { kind: "pct", pct: null, amt: "—", reset: "—", resetAt: null };
  }
  const tight = entries.find((e) => e.pct === tightest) || entries[0];
  return { kind: "pct", pct: `${tightest}%`, amt: `余 ${tight.pct === null || tight.kind === "bal" ? tight.remain : `${100 - tightest}%`}`, reset: tight.reset, resetAt: tight.resetAt ?? null };
}
...
// （主机/基础路径策略见文件开头的 OFFICIAL_ENDPOINTS）
...
const makeAnthropicOrgQuery = (kind) => wrapQuery(async (cfg) => {
  ...
  for (const bucket of buckets) {
    ...
    for (const row of rows) {
      hadRows = true;
      if (kind === "cost") {
        const cents = parseNumber(row?.amount);
...
async function queryOpenCode(cfg) {
      ...
      if (usage && typeof usage === "object") {
        for (const [key, label] of [["rolling", "5h 滚动"], ["weekly", "周用量"], ["monthly", "月用量"]]) {
          ...
          entries.push({
            ...
            pct,
            reset: formatReset(resetsAt),
            resetAt: resetAtMs(resetsAt),
            note: w.status || "",
          });
          if (pct !== null && (tightest === null || pct > tightest)) tightest = pct;
        }
  ...
  if (cfg.allowanceToken) {
    try {
      ...
      for (const m of meters) {
        ...
        const meterResetsAt = parseDate(m.resetsAt);
        entries.push({
          ...
          pct,
          reset: formatReset(meterResetsAt),
          resetAt: resetAtMs(meterResetsAt),
          note: "",
        });
        if (pct !== null && (tightest === null || pct > tightest)) tightest = pct;
      }
  ...
  if (!entries.length) return errResult("no-data", "OpenCode 未返回可识别的用量或额度");
  return { state: stateFromPct(tightest, cfg.warnPct, cfg.critPct), entries, headline: makePctHeadline(tightest, entries) };
}
...
// 同一次多端点、分页查询共享取消信号；直接测试连接也有总时限。
for (const provider of Object.values(PROVIDERS)) {
  const query = provider.query;
  provider.query = (config, { signal = AbortSignal.timeout(120_000) } = {}) =>
    queryContext.run(signal, () => query(config));
}
