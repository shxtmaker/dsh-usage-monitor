...
export function createScheduler({ providers, getConfig, onRecord, now = Date.now }) {
  const slots = Object.fromEntries(Object.keys(providers).map((id) => [id, {
    ...
    inFlight: null, controller: null, configKey: null, revision: 0,
  }]));
  let disposed = false;
  function sync(id) {
    const slot = slots[id];
  ...
  function run(id, { force = false, interval = 60_000 } = {}) {
    ...
    const pending = Promise.resolve().then(() => providers[id].query(config, {
      ...
      if (slot.inFlight === pending) { slot.inFlight = null; slot.controller = null; }
    });
  ...
  return {
    run,
    getState(id) {
      const { last, failures, nextAt, lastSuccessAt, inFlight } = slots[id];
      return { last: structuredClone(last), failures, nextAt, lastSuccessAt, inFlight: !!inFlight };
    },
    sync: () => Object.keys(providers).forEach(sync),
    tick(interval) { for (const id of Object.keys(providers)) run(id, { interval }); },
    dispose() { disposed = true; for (const slot of Object.values(slots)) slot.controller?.abort(); },
  };
}
