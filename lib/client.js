  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require) => {
      ...
      function SettingsModal({ t, onClose }) {
        ...
        React.useEffect(() => {
          ...
          getState().then((s) => {
            ...
            for (const sup of s?.suppliers || []) {
              ...
              // 按供应商 needs 元数据初始化每个字段（secret 留空 = 保持原值）
              for (const need of sup.meta?.needs || []) entry[need.key] = "";
              init.suppliers[sup.id] = entry;
            }
            setForm(init);
          });
          return () => { alive = false; };
        ...
        // 字段标签：apiKey 用凭据类别文案；allowance/orgId 用固定文案；其余回退服务端 label
        ...
        const needsInputs = (sup) =>
          (sup.meta?.needs || []).map((need) =>
            h("label", { key: need.key }, needsLabel(sup, need),
              h("input", {
                ...
                value: (form.suppliers[sup.id] && form.suppliers[sup.id][need.key]) || "",
                onChange: (e) => set(sup.id, need.key, e.target.value),
              }),
            ),
          );
        ...
        const groups = (state.suppliers || []).map((s) =>
          h("div", { key: s.id, className: "s-group" },
            ...
            (s.meta?.credentialClass && s.meta.credentialClass !== "api-key")
              ? h("div", { className: "pnote", style: { marginBottom: 8 } },
                  t("manualCredential", { label: t(s.meta.credentialLabelKey || "apiKey") }))
              : null,
            needsInputs(s),
        ...
        const save = () => {
          ...
          for (const s of state.suppliers || []) {
            ...
            for (const need of s.meta?.needs || []) {
              const v = f[need.key];
              if (need.key === "apiKey" || need.secret) {
                // secret：留空 = 保持不变（不支持在 UI 清除，见 README 限制）
