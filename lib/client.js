...
.qm-settings .s-saved{color:var(--dsw-alias-state-success-primary,#34d399);font-size:11px;align-self:center;}
      ...
      function Widget({ data, onOpenPopup, onOpenDetail, onOpenSettings, onRefresh }) {
        ...
        return h("div", { "data-dsh-qm-widget": "" },
          h("div", { className: "qm-head" },
            h("b", null, "用量"),
            h("span", null, `每 ${data?.poll?.intervalSeconds ?? 60}s`,
      ...
      function Settings({ data, onClose, onSave, onTest }) {
        ...
        const save = () => {
          const patch = { intervalSeconds: form.intervalSeconds, suppliers: {} };
          for (const s of supps) {
        ...
        const mount = () => {
          ...
          const mountInto = (column, fallback = false) => {
            ...
            const applyLayout = () => {
              ...
              const next = findBottomArea(column);
              if (next && next !== anchor) {
                anchor = next;
            ...
            const mo = new MutationObserver(applyLayout);
            mo.observe(column, { childList: true, subtree: true, attributes: true, attributeFilter: ["class", "style"] });
            layoutDispose = () => {
              ro.disconnect();
              mo.disconnect();
