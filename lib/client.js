// lib/client.js — DSH Web 客户端半（浏览器，经 /plugins/dsh-usage-monitor/client.js 加载）
//
// 渲染契约（v0.2 彻底重构版）：
...
//      宽栏紧凑条显示「当前显示页」的最近一次调用：显示页 = Session Controller 的
//      sessions.list.current（官方服务订阅，切会话即时重渲并按新会话 id 拉 /state），
//      多个会话并行用不同供应商时互不串页。
...
(function () {
  window.__ModuleLoader__.load({
    ...
    factory: (require) => {
      ...
      const POLL_MS = 10_000; // 客户端自身状态拉取节奏（宿主另有 10–3600s 取数轮询；10s 让页内新调用尽快反映）
      ...
      // 「最近一次调用」的相对时间（小组件 meta；不引入 moment 类依赖）
      const fmtAge = (ms, t) => {
      ...
      const stateToken = (s) => {
        ...
        if (!Number.isFinite(pct)) return "stateOk";
        return pct >= (s.critPct ?? 95) ? "stateCrit" : pct >= (s.warnPct ?? 80) ? "stateWarn" : "stateOk";
      };
      ...
      const getState = (sessionId) =>
        fetch(`${API}/state${queryOf(sessionId)}`, { cache: "no-store" }).then((r) => r.json());
      const post = (path, body, sessionId) =>
        fetch(`${path}${queryOf(sessionId)}`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify(body || {}),
        }).then(async (r) => {
          const result = await r.json();
          if (r.ok === false || result?.ok === false) throw new Error(result?.error || `HTTP ${r.status}`);
          return result;
        });
      ...
      function useQuotaState(sessionId) {
        ...
        const load = React.useCallback(async () => {
          ...
          try {
            ...
            if (version !== requestVersion.current || payload?.ok === false) return;
        ...
        const refresh = React.useCallback(async () => {
          const version = ++requestVersion.current;
          try {
            const next = await post(`${API}/refresh`, undefined, sessionId);
            if (version === requestVersion.current && next?.ok !== false) {
              cacheState(key, next);
              setState({ key, payload: next });
            }
          } catch (error) {
            console.error("[quota-monitor] refresh failed", error);
          }
        }, [sessionId, key]);
        // 渲染层按 key 对齐：切页瞬间若缓存命中立即显示该页数据，否则显示空（绝不残留上一页的供应商）
        const shown = state && state.key === key ? state.payload : (stateCache.get(key) || null);
        return { state: shown, refresh };
      }

      // ---------- 小组件：侧边栏脚部槽位（sidebar.footer.action） ----------
      // wide=true：紧凑条主显示改为「当前显示页（当前选中的会话）最近一次调用的
      // 模型供应商」—— label（在用）+ 供应商 · 模型 + 相对时间。sessionId 由
      // FooterSlotWithSession 注入（sessions.list.current）；限额仍在 Popover/详情承载。
      ...
      function FooterWidget({ wide, t, sessionId }) {
        ...
        const { state, refresh } = useQuotaState(sessionId);
        ...
        const stripTitle = activeText ? t("usingLabel") : t("title");
        ...
        const meta = [
          ...
          active ? fmtAge(Date.now() - active.at, t) : null,
        ...
        return h(React.Fragment, null,
          ...
          h("div", { "data-qm-linerow": "" },
            h("button", {
              ...
              h("span", { className: `qm-dot ${dotState}` }),
              h("span", { className: "qm-strip-title" }, stripTitle),
      ...
      function SettingsModal({ t, onClose }) {
        ...
        const saveSupplier = async () => {
          ...
          if (!sup) return;
          try {
            const p = supplierPatch(sup, editor.form);
        ...
        const runTest = (id) => {
          ...
          const sup = (state?.suppliers || []).find((s) => s.id === id);
          const config = sup && editor?.supplierId === id ? supplierPatch(sup, editor.form) : undefined;
        ...
        const runRescan = async () => {
          ...
          try {
            ...
            console.error("[quota-monitor] rescan failed", error);
            ...
            h("div", { className: "s-group qm-quota-preview" },
              ...
              (sup.entries && sup.entries.length)
                ? h("div", { className: "qm-quota-cards" },
                    sup.entries.map((e) => h(EntryCard, { key: e.name, e, t })))
...
@keyframes qm-fresh{from{background:rgba(79,140,255,.30);}to{background:transparent;}}
      ...
      function ensureStyle() {
        const tagId = "dsh-usage-monitor/widget.css";
      ...
      function apply(ctx) {
        ...
        try {
          ...
          console.warn("[quota-monitor] sessions service unavailable, sidebar shows global latest", error);
        }
