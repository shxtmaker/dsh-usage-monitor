...
//   - 当日消耗量：session/event 的 usage 按 provider 归并，自然日 00:00 重置
...
const runtime = {
  ...
  today: {}, // supplierId -> { dayKey, tokens }
...
function collectTraffic() {
  ...
  const currentIds = enabledIds.filter((id) => {
  ...
  const todayOf = (id) => {
    ...
    const rec = runtime.today[id];
    if (!rec || rec.dayKey !== dayKey) return null;
    return rec.tokens;
  };
  return { currentIds, todayOf };
}
...
function buildStatePayload() {
  const { currentIds, todayOf } = collectTraffic();
  return {
    ok: true,
  ...
  ctx.on("session/event", (session, event) => {
    ...
    try {
      ...
      if (type === "assistant/message") {
        provider = data?.source?.provider || null;
        usage = data?.usage || null;
      } else if (type === "assistant/chunk") {
        provider = data?.provider || data?.source?.provider || null;
        usage = data?.usage || null;
      } else if (type === "request/header") {
        provider = data?.config?.provider || null;
      }
      if (typeof provider === "string" && provider) {
        const seen = runtime.routeSeen[provider] || { lastSeenAt: 0, lastModel: null };
        seen.lastSeenAt = Date.now();
        const model = data?.config?.model || data?.model || data?.source?.model || null;
        if (model) seen.lastModel = model;
        runtime.routeSeen[provider] = seen;
      }
      const supplierId = supplierForRoute(provider);
      if (supplierId && usage && typeof usage === "object") {
        ...
        if (tokens > 0) {
          const dayKey = new Date().toLocaleDateString();
          const rec = runtime.today[supplierId] || { dayKey, tokens: 0 };
          if (rec.dayKey !== dayKey) {
            rec.dayKey = dayKey;
            rec.tokens = 0;
          }
          rec.tokens += tokens;
          runtime.today[supplierId] = rec;
        }
      }
    } catch {
      /* 事件折叠失败不影响主流程 */
