...
import { dayKeyOf, dshHome, acquireUsageStore, migrateUsageDir } from "./storage.js";

const NS = "dsh-token-quota";
/** 改名前（≤ v1.1.1）的 settings 命名空间，仅用于一次性迁移。 */
...
export function apply(ctx) {
  ...
  function buildStatePayload(session = null) {
    ...
    return {
      ...
      suppliers: Object.keys(PROVIDERS).map((id) => {
        ...
        const detectedNow = !!c.autoSource || !!runtime.harness[id] || runtime.detect.found.includes(id);
  ...
  // 插件改名（dsh-usage-monitor → dsh-token-quota）的一次性 settings 迁移：
  // 新命名空间还是空的、而旧命名空间有用户配置（含已填的密钥）时，整段拷过去。
  // 绝不删旧的——宁可留一份孤儿配置，也不在迁移失败时让用户重填密钥。
  async function migrateLegacyNamespace() {
    try {
      if (typeof ctx.settings?.describe !== "function" || typeof ctx.settings?.update !== "function") return;
      if (settingsUserHas(ctx, NS)) return;                      // 新命名空间已有用户配置：不动
      const legacy = ctx.settings.describe().find((d) => d.ns === LEGACY_NS);
      const user = legacy?.user;
      if (!user || typeof user !== "object" || Object.keys(user).length === 0) return;
      await ctx.settings.update(NS, user);
      ctx.logger?.info?.(`[${NS}] 已从旧命名空间 ${LEGACY_NS} 迁移配置（旧数据保留，可手动清理）`);
    } catch (error) {
      ctx.logger?.warn?.(`[${NS}] 旧命名空间迁移失败，保持原样: %s`, error?.message || error);
    }
  }

  const scope = ctx.settings.register(NS, Config, { base: {} });
  runtime.config = scope.get() || {};
  void migrateLegacyNamespace();
  ...
  runtime.usageFile = migrateUsageDir(dshHome());
  ...
  const scanHarness = async () => {
    ...
    try {
      ...
      ctx.logger?.warn?.("[dsh-token-quota] 自动探测失败: %s", error?.message || error);
    }
  };
  ...
  const syncHarness = () => {
    ...
    if (scanPromise) { scanAgain = true; return scanPromise; }
    scanPromise = (async () => {
      do { scanAgain = false; await scanHarness(); } while (scanAgain && !disposed);
  ...
  // 手动「重新扫描」入口：与周期自动探测同一实现；单实例并发（进行中复用同一 Promise），
  // 探测版本门禁保证迟到结果不覆盖新结果
  const triggerScan = () => scanPromise || syncHarness();
