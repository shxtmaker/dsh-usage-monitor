...
// 只统计能映射到插件供应商的 route（session/event 已把 route/model 折叠进 runtime.routeSeen）；
// 与 enabled/current 过滤解耦：DSH 真在用的供应商即使本插件未启用也照实显示。
function computeActiveSupplier() {
  let best = null; // { supplierId, model, at }
  for (const [route, seen] of Object.entries(runtime.routeSeen)) {
