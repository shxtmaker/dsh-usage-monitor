...
- **小组件（sidebar）**：状态点 + 名称 + 百分比（余额类显示金额）+ 当日 token 消耗量；点击弹 popover（限额项/重置/错误/上次刷新）；只显示**当前供应商**（DSH 启用 ∩ 近期流量，「无 DSH 路由的供应商恒为候选」；只有真实 session/event 才算流量信号，冷启动未调用的会话按启用清单兜底显示，避免小组件「暂无供应商」）；**位置自适应**——插在侧栏底部按钮区上方（8px 间隙永不重叠），内容超高内部滚动，侧栏收起为窄栏时自动隐藏
